...
- **`Stylus/learnscripture/catppuccin.user.less`** — Less, same versioned-lib convention as okta/esv. Personal style, not intended for upstreaming. LearnScripture.net paints each of its four built-in themes (Calm, Bubblegum, Bubblegum 2, Space) from ~18 custom properties on `body[data-theme=<name>]`, so the style redeclares that block on the same selectors and wins on source order — no `!important` for tokens. Three things worth knowing before editing — see `Stylus/learnscripture/README.md`:
